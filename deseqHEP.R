...
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- paste(vsd$condition, vsd$replicate, vsd$condition, sep="-")
...
# VST counts
vsd <- vst(dds_hep_norep1) 

# PCA plot
rld <- rlog(dds_hep_norep1, blind=FALSE)
plotPCA(vsd, intgroup=c("condition", "replicate"))
...
# cooks distance
par(mar=c(8,5,2,2))
boxplot(log10(assays(dds_hep_norep1)[["cooks"]]), range=0, las=2)        

# sample distance heatmap
sampleDists <- dist(t(assay(vst(dds_hep_norep1))))
sampleDistMatrix <- as.matrix(sampleDists)
pheatmap::pheatmap(sampleDistMatrix, labels_row=colnames(dds_hep_norep1))
...
# merge with conversion info
resLFChep_final_norep1 <- merge(resLFChep_df_norep1, conversion_unique, by = "Gene.stable.ID", all.x = TRUE)
...
# ------------------------ see what genes are the most variable across EtOH samples -------- #
vsd_mat <- assay(vsd)
# Calculate variance
gene_vars <- apply(vsd_mat, 1, var)
top_genes <- head(sort(gene_vars, decreasing = TRUE), 20)
pheatmap(vsd_mat[names(top_genes), ], scale = "row", show_rownames = TRUE)
